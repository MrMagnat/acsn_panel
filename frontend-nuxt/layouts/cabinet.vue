  ...
  <div class="min-h-screen flex bg-surface">
    ...
    <aside class="w-60 bg-white border-r border-gray-100 flex flex-col shrink-0">
      ...
      <nav class="flex-1 px-3 py-4 space-y-1">
        <NuxtLink to="/cabinet/office" class="nav-link" active-class="nav-link--active">
          <span>🏠</span> Мой офис
        </NuxtLink>
        <NuxtLink to="/cabinet/tools" class="nav-link" active-class="nav-link--active">
          <span>🔧</span> Инструменты
        </NuxtLink>
        <NuxtLink to="/cabinet/settings" class="nav-link" active-class="nav-link--active">
          <span>⚙️</span> Настройки
        </NuxtLink>
        <template v-if="auth.user?.is_admin">
          <div class="border-t border-gray-100 my-2"></div>
          <NuxtLink to="/admin/users" class="nav-link text-purple-600" active-class="nav-link--active">
