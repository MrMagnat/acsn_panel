      ...
      <div class="lg:col-span-1 space-y-2">
        ...
        >
          <div class="flex items-center justify-between">
            ...
            <span :class="t.is_active ? 'badge-active' : 'badge-inactive'">{{ t.is_active ? 'Активен' : 'Откл.' }}</span>
          </div>
      ...
      <div v-if="selected" class="lg:col-span-2 card p-0 overflow-hidden">
        ...
        <div class="p-6">
          <div v-if="activeTab === 'basic'" class="space-y-4">
            ...
            <div><label class="label">Описание</label><textarea v-model="form.description" class="input resize-none h-20"></textarea></div>
            <div>
          ...
          <div v-else-if="activeTab === 'prompt'" class="space-y-4">
            <div><label class="label">Системный промпт</label><textarea v-model="form.prompt" class="input resize-none h-40 font-mono text-sm"></textarea></div>
            <div><label class="label">Скиллы / Знания</label><textarea v-model="form.skills" class="input resize-none h-24 font-mono text-sm"></textarea></div>
          </div>

          <div v-else-if="activeTab === 'tools'" class="space-y-3">
            <div v-for="tool in allTools" :key="tool.id"
          ...
          <div v-else-if="activeTab === 'energy'" class="space-y-4">
            ...
            <div class="p-4 bg-gray-50 rounded-lg text-sm text-gray-600">
              ...
              <div v-for="tid in form.tool_ids" :key="tid">{{ getToolName(tid) }}: ⚡{{ getToolCost(tid) }}</div>
              <div class="border-t border-gray-200 mt-2 pt-2 font-semibold">Итого: ⚡{{ totalEnergy }}</div>
            </div>
          </div>
        </div>
      </div>
...
const form = ref({ name: '', description: '', llm_url: '', llm_model: '', llm_token: '', prompt: '', skills: '', is_active: true, energy_per_chat: 5, tool_ids: [] })
...
onMounted(async () => {
  const [tRes, toolsRes] = await Promise.allSettled([adminApi.getTemplates(), adminApi.getTools()])
...
function selectTemplate(t) {
  ...
  form.value = { name: t.name, description: t.description, llm_url: t.llm_url, llm_model: t.llm_model ?? '', llm_token: t.llm_token ?? '', prompt: t.prompt, skills: t.skills, is_active: t.is_active, energy_per_chat: t.energy_per_chat, tool_ids: t.tools?.map((tool) => tool.id) ?? [] }
}
...
async function deleteTemplate() {
  if (!selected.value.id || !confirm('Удалить шаблон?')) return
  try {
...
function getToolName(id) { return allTools.value.find((t) => t.id === id)?.name ?? id }
function getToolCost(id) { return allTools.value.find((t) => t.id === id)?.energy_cost ?? 0 }
</script>
