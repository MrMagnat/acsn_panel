      ...
      <div class="lg:col-span-1 space-y-2">
        <div v-for="tool in tools" :key="tool.id"
          ...
          <div class="flex items-center justify-between">
            ...
            <span :class="tool.is_active ? 'badge-active' : 'badge-inactive'">{{ tool.is_active ? 'Активен' : 'Откл.' }}</span>
          </div>
      ...
      <div v-if="selected" class="lg:col-span-2 card p-6 space-y-5">
        <div class="flex items-center justify-between pb-3 border-b border-gray-100">
          ...
          <div class="flex gap-2">
            ...
            <button class="btn-primary text-xs" @click="saveTool" :disabled="saving">{{ saving ? 'Сохраняю...' : 'Сохранить' }}</button>
          </div>
        </div>

        <div class="grid grid-cols-2 gap-4">
          <div><label class="label">Название</label><input v-model="form.name" class="input" /></div>
          <div><label class="label">Стоимость энергии</label><input v-model.number="form.energy_cost" type="number" min="0" class="input" /></div>
        </div>
        <div><label class="label">Описание</label><textarea v-model="form.description" class="input resize-none h-16" placeholder="Что делает инструмент..."></textarea></div>
        <div>
          <label class="label">Когда использовать <span class="text-xs font-normal text-gray-400 ml-1">(подсказка для ИИ)</span></label>
          <textarea v-model="form.trigger_hint" class="input resize-none h-16" placeholder="Используй когда пользователь просит..."></textarea>
        </div>
        <div><label class="label">Webhook URL</label><input v-model="form.webhook_url" class="input font-mono text-sm" placeholder="https://..." /></div>
        ...
        <div>
          ...
          <div class="space-y-3">
            <div v-for="(field, idx) in form.fields" :key="idx" class="p-3 rounded-lg bg-gray-50 border border-gray-100 space-y-2">
...
const form = ref({ name: '', description: '', trigger_hint: '', webhook_url: '', is_active: true, energy_cost: 10, fields: [] })
...
function selectTool(tool) {
  selected.value = tool
  form.value = { name: tool.name, description: tool.description, trigger_hint: tool.trigger_hint ?? '', webhook_url: tool.webhook_url, is_active: tool.is_active, energy_cost: tool.energy_cost, fields: tool.fields?.map((f) => ({ ...f })) ?? [] }
}
