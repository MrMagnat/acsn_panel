...
def _build_system_prompt(agent, configured_tools: list, agent_skills: list = None) -> str:
    ...
    scope = ""
    if configured_tools:
        tool_names = ", ".join(f'«{at.tool.name}»' for at in configured_tools)
        scope = f" Твои задачи ограничены следующими инструментами: {tool_names}."
    parts.append(
        "## Базовые правила\n"
        f"- Ты — специализированный AI-ассистент.{scope}\n"
        "- СТРОГО отказывай в любых запросах вне твоей специализации и инструментов. Не помогай с общими вопросами, кодом, текстами и другими задачами не по теме.\n"
        "- Если пользователь просит что-то вне твоей темы — коротко скажи, что ты специализируешься только на своих задачах.\n"
        "- НИКОГДА не перечисляй возможности списком. Если спрашивают 'что ты умеешь' — одной фразой назови специализацию.\n"
    ...
    if configured_tools:
        ...
        parts.append(
            ...
            "1. Никогда не вызывай инструмент без явного подтверждения пользователя.\n"
            "2. Перед вызовом — покажи список всех полей и их значений (текущих или из сообщения пользователя) одним блоком и спроси: 'Всё верно? Запускаем?'\n"
            "   - Если какого-то обязательного поля нет — сначала спроси его, потом показывай итоговый список.\n"
            "   - Не задавай вопросы по одному полю за раз — собери всё и покажи сразу.\n"
            "3. Как только пользователь подтвердил ('да', 'верно', 'запускай', 'ок' и т.п.) — вызывай инструмент немедленно.\n"
            "4. После вызова кратко прокомментируй результат.\n"
