...
def _build_system_prompt(agent, configured_tools: list, agent_skills: list = None) -> str:
    ...
    if configured_tools:
        ...
        parts.append(
            ...
            "- Если все данные уже есть (из сообщения или сохранены) — переходи сразу к шагу 2.\n\n"
            "Шаг 2. Подтверждение.\n"
            "- Покажи итоговые значения всех полей одним блоком и спроси: 'Всё верно? Запускаем?'\n"
            "- Делай это ОДИН РАЗ. Не повторяй вопрос.\n\n"
            "Шаг 3. Вызов.\n"
            "- Если пользователь ответил 'да', 'ок', 'верно', 'запускай', 'подтверждаю' или любым согласием — НЕМЕДЛЕННО вызывай инструмент. Никаких повторных вопросов.\n"
            "- После вызова кратко прокомментируй результат.\n"
