        ...
        try:
            ...
            if llm_response.get("tool_call"):
                ...
                # --- Пользовательский инструмент (webhook) ---
                    ...
                    # Серверная проверка подтверждения: если последнее сообщение пользователя
                    # не является подтверждением — не запускаем инструмент, просим подтвердить
                    _CONFIRM_WORDS = {
                        "да", "ок", "ок.", "ладно", "хорошо", "верно", "всё верно", "все верно",
                        "запускай", "запусти", "подтверждаю", "подтверждаю.", "yes", "ok", "run",
                        "давай", "выполняй", "выполни", "конечно", "согласен", "согласна",
                    }
                    last_user_lower = content.strip().lower()
                    is_confirmed = (
                        last_user_lower in _CONFIRM_WORDS
                        or any(last_user_lower.startswith(w) for w in (
                            "да,", "ок,", "да!", "ок!", "да.", "да ", "ок ", "верно,", "верно.",
                            "подтверждаю", "запускай", "запусти", "давай", "выполняй",
                        ))
                    )

                    if matched_at and not is_confirmed:
                        # Формируем список параметров для показа пользователю
                        merged_preview = {**(matched_at.field_values or {}), **tool_args}
                        params_text = "\n".join(
                            f"• **{k}**: {v}" for k, v in merged_preview.items() if v not in (None, "")
                        ) or "(параметры не требуются)"
                        assistant_content = (
                            f"Запускаю **{tool_name}** со следующими параметрами:\n\n"
                            f"{params_text}\n\n"
                            f"Всё верно? Запускаем? _(ответьте «да» для подтверждения)_"
                        )
...
def _build_system_prompt(agent, configured_tools: list, agent_skills: list = None) -> str:
    ...
    parts.append(
        ...
        "- Если спрашивают 'что ты умеешь' / 'расскажи о себе' / 'какие скиллы' — кратко опиши свою специализацию и перечисли доступные инструменты и скиллы по названию.\n"
        "- Отказывай в задачах полностью вне твоей специализации.\n"
        "- КРИТИЧНО: если для выполнения задачи есть подходящий инструмент — ВСЕГДА используй инструмент через function call. "
        "НИКОГДА не выполняй задачу текстом в чате, если есть инструмент для этого. "
        "Написать пост, отправить письмо, создать задачу, опубликовать контент — всё это делается через инструменты, не через текст."
    )
    ...
    if configured_tools:
        ...
        parts.append(
            ...
            "КРИТИЧНО: когда пользователь просит выполнить задачу — НЕ ДЕЛАЙ ЭТО ТЕКСТОМ. Используй инструмент.\n"
            "Пример: пользователь просит 'напиши пост' → ты собираешь параметры → вызываешь инструмент. НЕ пишешь пост в ответе.\n\n"
            "Шаг 1. Сбор данных.\n"
            "- Если обязательных полей не хватает — задай все недостающие вопросы ОДНИМ сообщением.\n"
            "- Если все данные уже есть — сразу вызывай инструмент через function call.\n\n"
            "Шаг 2. Вызов.\n"
            "- Как только все данные собраны — вызывай инструмент. Система покажет параметры и попросит подтвердить.\n"
            "- Если пользователь уже подтвердил ('да', 'ок', 'запускай') — инструмент выполнится немедленно.\n\n"
            "Шаг 3. Результат.\n"
            "- После получения результата — объясни его пользователю кратко.\n"
            "- Если инструмент вернул ошибку — сообщи и предложи решение.\n\n"
            "Дополнительно:\n"
            "- Если пользователь просит сохранить настройки — используй save_tool_settings.\n"
            "- Если пользователь просит настроить автозапуск — используй create_auto_trigger."
        )
