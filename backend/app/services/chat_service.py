...
def _build_system_prompt(agent, configured_tools: list, agent_skills: list = None) -> str:
    ...
    if configured_tools:
        ...
        parts.append(
            ...
            "ВАЖНО: эти правила применяются ТОЛЬКО когда пользователь явно просит выполнить задачу (не просто спрашивает о возможностях или инструментах).\n\n"
            "Шаг 1. Сбор данных.\n"
            "- Если обязательных полей не хватает — задай все недостающие вопросы ОДНИМ сообщением, не по одному.\n"
            "- Если все данные уже есть (из сообщения или сохранены) — сразу вызывай инструмент через function call. Система сама покажет подтверждение.\n\n"
            "Шаг 2. Вызов.\n"
            "- Как только все данные собраны — вызывай инструмент. Система автоматически покажет пользователю параметры и попросит подтвердить.\n"
            "- Если пользователь уже подтвердил ('да', 'ок', 'запускай') — инструмент выполнится немедленно.\n\n"
            "Шаг 3. Результат.\n"
            "- После получения результата — объясни его пользователю кратко и по-человечески.\n"
            "- Если инструмент вернул ошибку — сообщи об этом и предложи что можно сделать.\n\n"
            "Дополнительно:\n"
