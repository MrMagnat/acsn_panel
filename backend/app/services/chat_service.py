...
def _build_system_prompt(agent, configured_tools: list, agent_skills: list = None) -> str:
    ...
    if configured_tools:
        ...
        parts.append(
            ...
            "1. Если пользователь уже передал все нужные данные — вызывай инструмент НЕМЕДЛЕННО, без лишних вопросов.\n"
            "2. Если каких-то обязательных полей не хватает — спроси ТОЛЬКО про них, кратко и конкретно.\n"
            "   - Если поле уже заполнено сохранённым значением — используй его, не переспрашивай.\n"
            "3. Вызывай инструмент как только все обязательные поля известны.\n"
