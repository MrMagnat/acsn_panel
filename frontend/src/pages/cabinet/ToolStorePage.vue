<template>
    ...
    <div v-else id="tools-grid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      >
        <div class="flex items-start justify-between gap-2">
          <div class="flex items-center gap-1.5 min-w-0">
            ...
            <button
              ...
              @click.stop="openMaintenance(tool, null)"
            >🔧</button>
          </div>
        ...
        <button
          ...
          @click="tool.is_maintenance ? openMaintenance(tool, () => openRunModal(tool)) : openRunModal(tool)"
        >
          ▶ Запустить
        </button>
      </div>
  ...
  <MaintenanceModal
    :show="!!maintenanceTool"
    :label="maintenanceTool?.name || 'Инструмент'"
    @close="maintenanceTool = null; maintenanceCb = null"
    @continue="maintenanceCb?.(); maintenanceTool = null; maintenanceCb = null"
  />
  </div>
</template>
...
const maintenanceTool = ref(null)
const maintenanceCb = ref(null)

function openMaintenance(tool, cb) {
  maintenanceTool.value = tool
  maintenanceCb.value = cb
}
